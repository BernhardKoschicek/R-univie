...
# data(methodists)
# methodists 
meth <- load(file = "C:/Users/bkoschicek/Dropbox/Geschichte Studium/3_Sem/R-repo/05/methodists.rda")
meth
```
...
```{r}
select(methodists, year,meeting, members_total)
```
...
```{r}
methodists %>% filter(state == "New York", year == "1800")
```

(@) Which Methodist meetings had only black members?

```{r}
methodists %>% filter(members_white == 0, members_black > 0 )
```

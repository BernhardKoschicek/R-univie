...
names(song_rankings) <- c("Good Song", "ok song", "bad song", "okish song", "worst song")
song_rankings["Good Song"]
```
...
> 5 columns and 5705 rows. There are int and chr. But I have no idea what type rank is., 
...
```{r}
head(sort(as.Date(naval_promotions$date)), 1, na.rm = TRUE)
tail(sort(as.Date(naval_promotions$date)), 1, na.rm = TRUE)
...
```{r}
my_event <- list(
  title = "Dusan rise to power",
  year = 1331,
  region = "Macedonia",
  parents = c("Stefan Decanski", "Theodora Smilets of Bulgaria")
)

is.data.frame(my_event$title)
is.list(my_event)
...
```{r}
rank_captain <- naval_promotions[naval_promotions$rank == "captain", ]
head(rank_captain, 10)
```
...
prom_1800 = naval_promotions[year(ymd(naval_promotions$date, quiet=TRUE)) == 1800, ]
prom_1800
